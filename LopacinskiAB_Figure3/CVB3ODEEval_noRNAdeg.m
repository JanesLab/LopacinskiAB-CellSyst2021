function [MedianSolutions,MeanSolutions,UpperQuantileSolutions,LowerQuantileSolutions,SensitivitySolutions] = CVB3ODEEval_noRNAdeg(MOI,varargin)
...
%%%Set positive-strand degradation rate to zero on VROs
u_P_VRO = 0; %lognrnd(muFunc(0.23/VROVolConv,(CV*0.23/VROVolConv)^2),sigma); %(h^-1) 
%%%
u_N_cyt = lognrnd(muFunc(0.23,(CV*0.23)^2),sigma); %(h^-1) 
%%%Set negative-strand degradation rate to zero on VROs
u_N_VRO = 0; %lognrnd(muFunc(0.23/VROVolConv,(CV*0.23/VROVolConv)^2),sigma); %(h^-1) 
%%%
u_T_c = 0; %(h^-1) 
