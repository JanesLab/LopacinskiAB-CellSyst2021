function [t,TotalRNARatioSummary] = CVB3ODEEvalFig3E(MOI,varargin)
%Function: Stores all constants, initial values, runs the ODE, and calls other relevant functions
%%%THIS FUNCTION HAS BEEN RECONFIGURED TO REPORT ONLY SUMMARY STATISTICS OF
%%%RNA POSITIVE-NEGATIVE STRAND RATIOS
%
%INPUTS:
...
%%%Engable user-defined dsRNAMeasureEfficiency
    %dsRNAMeasureEfficiency:  The efficiency of detecting dsRNA intermediates by experimental methods.  Default = 0.
%%%
%
%OUTPUTS:
...
defaultCV = 0.05;
%%%Engable user-defined dsRNAMeasureEfficiency
defaultdsRNAMeasureEfficiency = 0;
%%%
...
%%%Engable user-defined dsRNAMeasureEfficiency
   addOptional(CVB3ODEEvalparser,'dsRNAMeasureEfficiency',defaultdsRNAMeasureEfficiency,validPercent);
%%%
...
%%%Engable user-defined dsRNAMeasureEfficiency
dsRNAMeasureEfficiency = CVB3ODEEvalparser.Results.dsRNAMeasureEfficiency;
%%%
...
%%%Incorporate dsRNAMeasureEfficiency
TotalPosStrands = InternalVolConv*(0*sum(PermuteSolutionsTensor(:,:,4:5),3) ... %bDAF, bDAF_Defective; assumed instantly removed before lysis
    ...
    + dsRNAMeasureEfficiency*PermuteSolutionsTensor(:,:,23) ... %R_Ip_VRO
...
TotalNegStrands = EncapsidatedNegStrandRatio*(InternalVolConv*(0*sum(PermuteSolutionsTensor(:,:,4:5),3) ... %bDAF, bDAF_Defective; assumed instantly removed before lysis
    ...
    + dsRNAMeasureEfficiency*PermuteSolutionsTensor(:,:,24); %R_In_VRO
%%%
...
TotalRNARatioSummary(:,1) = median(TotalRNARatio,2);
...
TotalRNARatioSummary(:,2) = quantile(TotalRNARatio,UpperQuantile,2);
...
TotalRNARatioSummary(:,3) = quantile(TotalRNARatio,LowerQuantile,2);
...
TotalRNARatioSummary(:,4) = mean(TotalRNARatio,2);
