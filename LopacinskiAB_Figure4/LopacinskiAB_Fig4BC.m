%% Figure 4B and 4C: Model calibration by quantitative immunofluorescence of VP1 and eIF4G protein dynamics.
...
% Execute simulations
[medianResultsTable,meanResultsTable,Q95ResultsTable,Q05ResultsTable, ~ ] = CVB3ODEEval(10,'MaxTime',8,'PlotResults','off');

%Estimate the adjustment constant to convert pixel intensity to approximate intracellular concentration. 
%Adjustment constant will be estimated using linear regression. Regressionmodel : medianModelPrediction ~ adjustmentCoefficient * experimentalData

%Find the median scaling coefficient for VP1 timecourse data
modelTimePointIndexes = [1,61,121,241,301,361,480];
VP1DataScaling = experimentalData{:,2:5}./medianResultsTable{modelTimePointIndexes,60};
adjustmentCoefficientVP1 = median(VP1DataScaling,'all','omitnan');
...
adjFitClveIF4G = fitlm(reshape(experimentalData{:,10:13},height(experimentalData(:,10:13))*width(experimentalData(:,10:13)),1), ...
    repmat(medianResultsTable{modelTimePointIndexes,'Viral Ribosomes'} - medianResultsTable{1,'Viral Ribosomes'}, ...
...
adjustedClveIF4G = medianResultsTable{1,'Viral Ribosomes'} + experimentalData{:,10:13}*adjustmentCoefficientClveIF4G;
...
subplot(1,2,1)
 modelTime = medianResultsTable{:,1};
 modelMedian = medianResultsTable{:,60};
 ...
 modelMedian(modelMedian==0)=1e-4;
 modelUQ(modelUQ==0)=1e-4;
 modelLQ(modelLQ==0)=1e-4;
 
 semilogy(modelTime,modelMedian,'Color','#7E2F8E','LineWidth',1)
 ylim([0.0001,10000])
 hold on
 ...
 subplot(1,2,2)
 modelTime = medianResultsTable{:,1};
 modelMedian = medianResultsTable{:,'Viral Ribosomes'};
 modelUQ = Q95ResultsTable{:,'Viral Ribosomes'};
 modelLQ = Q05ResultsTable{:,'Viral Ribosomes'};
 
 %remove absolute zeros from model and data so that it can plotted on a semilog axis.
 modelMedian(modelMedian==0)=1e-6;
 modelUQ(modelUQ==0)=1e-6;
 modelLQ(modelLQ==0)=1e-6;
 
 plot(modelTime,modelMedian,'Color','#7E2F8E','LineWidth',1)
 ylim([4,11])
 hold on
 patch([modelTime' fliplr(modelTime')], [modelLQ' fliplr(modelUQ')],'k','EdgeColor','none', 'FaceColor','#7E2F8E','FaceAlpha',.25)
