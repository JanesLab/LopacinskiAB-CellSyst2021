function [MedianSolutions,MeanSolutions,UpperQuantileSolutions,LowerQuantileSolutions,SensitivitySolutions] = CVB3ODEEvalFigS2A(MOI,varargin)
...
%%%Enable CAR and Recycling as user-defined inputs
    %CAR: CAR initial condition.  Default = 5.5e6.
    %Recycling:  Default = 'off'.
%%%
%
%OUTPUTS:
...
%%%Enable CAR and Recycling as user-defined inputs
   addOptional(CVB3ODEEvalparser,'CAR',defaultCAR,validPosNum);
   addOptional(CVB3ODEEvalparser,'Recycling',defaultRecycling,validSwitchString);
%%%
...
%%%Enable CAR and Recycling as user-defined inputs
CAR_IC = CVB3ODEEvalparser.Results.CAR;
Recycling = CVB3ODEEvalparser.Results.Recycling;
%%%
...
%%%Enable CAR as user-defined input
uCAR = CAR_IC / (6.022e23) / (CVB3diameter/1000 * CellSurfaceArea) * 1e24; %(nM) Unbound CAR in tight junctions
%%%
...
%%%Call alternative CVB3ODEfunc if Recycling is on
if strcmpi(Recycling,'on')
    [t,SolutionsMatrix] = ode15s(@(t,y)CVB3ODEfuncFigS2A(t,y,Constants,IFNSwitch,VirResponse,IFNStimulation,IFNStimulationTime),TSPAN,InitVals,Options); %Solving the ODE
else
...
%%%Call alternative CVB3ODEfunc if Recycling is on
    if strcmpi(Recycling,'on')
        [SensTable]= SensitivityAnalysisfunc(@CVB3ODEfuncFigS2A,LabeledConstants,LabeledInitVals,SensitivityAnalysisOutput,OtherSpecies,ScalingFactor,MaxScalingOrder,IFNSwitch,VirResponse,IFNStimulation,IFNStimulationTime,MaxTime,Options);
    else
