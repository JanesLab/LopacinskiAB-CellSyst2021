function [MedianSolutions,MeanSolutions,UpperQuantileSolutions,LowerQuantileSolutions,SensitivitySolutions] = CVB3ODEEvalS4LimConditions(MOI,varargin)
...
%%%Turn off degradaion of all RNA species
u_P_cyt = 0; %(h^-1) 
u_P_VRO = 0; %(h^-1) 
u_N_cyt = 0; %(h^-1) 
u_N_VRO = 0; %(h^-1) 
%%%
u_T_c = 0; %(h^-1) 
%%%Turn off degradation of all protein species
u_VirProt_cyt = 0; %(h^-1)
u_VirProt_VRO = 0; %(h^-1) 
%%%
...
%%%No defective viral particles
uCVB3_Defective = 0; %(nM) Unbound defective CVB3 virions at cell surface
%%%
...
%%%Do not count external viral capsid protein
TotalP1 = 0*InternalVolConv*60*(sum(PermuteSolutionsTensor(:,:,4:5),3) ... %bDAF, bDAF_Defective
    + 0*sum(PermuteSolutionsTensor(:,:,7:8),3) ... %bDAF_TJ, bDAF_Defective_TJ
    + 0*sum(PermuteSolutionsTensor(:,:,12:13),3)) ... %bCAR, bCAR_Defective
    + 0*60*sum(PermuteSolutionsTensor(:,:,14:15),3) ...  %R_p_endo, R_p_endo_Defective
...
%%%Keep track of individual species
TotalP2Summary(:,1) = median(TotalP2,2);
TotalP3Summary(:,1) = median(TotalP3,2);
TotalP4Summary(:,1) = median(TotalP4,2);
%%%
...
%%%Keep track of individual species
TotalP2Summary(:,2) = quantile(TotalP2,UpperQuantile,2);
TotalP3Summary(:,2) = quantile(TotalP3,UpperQuantile,2);
TotalP4Summary(:,2) = quantile(TotalP4,UpperQuantile,2);
%%%
...
%%%Keep track of individual species
TotalP2Summary(:,3) = quantile(TotalP2,LowerQuantile,2);
TotalP3Summary(:,3) = quantile(TotalP3,LowerQuantile,2);
TotalP4Summary(:,3) = quantile(TotalP4,LowerQuantile,2);
%%%
...
%%%Keep track of individual species
TotalP2Summary(:,4) = mean(TotalP2,2);
TotalP3Summary(:,4) = mean(TotalP3,2);
TotalP4Summary(:,4) = mean(TotalP4,2);
%%%
...
ResultsLabels = {'Time'...
    ...
    'Total +ssRNA' 'Total -ssRNA' 'Total dsRNA' 'Total Capsid Protein' 'Total 3Dpol' 'Total Protease' 'Total 2C ATPase'};
...
MedianTable = table(t,...%Time
    ...
    TotalPosStrandsSummary(:,1), TotalNegStrandsSummary(:,1), TotalDoubleStrandsSummary(:,1), TotalP1Summary(:,1),TotalP2Summary(:,1),TotalP3Summary(:,1),TotalP4Summary(:,1),...%Summary species
...
UpperQuantileTable = table(t,...%Time
    ...
    TotalPosStrandsSummary(:,2), TotalNegStrandsSummary(:,2), TotalDoubleStrandsSummary(:,2), TotalP1Summary(:,2),TotalP2Summary(:,2),TotalP3Summary(:,2),TotalP4Summary(:,2),...%Summary species
    'VariableNames',ResultsLabels);
...
LowerQuantileTable = table(t,...%Time
    ...
    TotalPosStrandsSummary(:,3), TotalNegStrandsSummary(:,3), TotalDoubleStrandsSummary(:,3), TotalP1Summary(:,3),TotalP2Summary(:,3),TotalP3Summary(:,3),TotalP4Summary(:,3),...%Summary species
    'VariableNames',ResultsLabels);
...
MeanTable = table(t,...%Time
    ...
    TotalPosStrandsSummary(:,4), TotalNegStrandsSummary(:,4), TotalDoubleStrandsSummary(:,4), TotalP1Summary(:,4),TotalP2Summary(:,4),TotalP3Summary(:,4),TotalP4Summary(:,4),...%Summary species
    'VariableNames',ResultsLabels);
%%%
